...
local function get_name()
    local n = trim(reaper.GetExtState("RADIO_MARKER", "P2_NAME"))
    if n == "" then n = "Intervenant 2" end
    return n
end
...
local txt = reaper.GetExtState("RADIO_MARKER", "P2_NOM")
local name = trim(txt)
if name == "" then
    name = "NOTE"
end
reaper.DeleteExtState("RADIO_MARKER", "P2_NOM", false)

local position = reaper.GetPlayPosition()
local color = color_from_ext("P2_NOM_COLOR", "FF8C00")
