...
local function get_name()
    local n = trim(reaper.GetExtState("RADIO_MARKER", "P4_NAME"))
    if n == "" then n = "Intervenant 4" end
    return n
end
...
local txt = reaper.GetExtState("RADIO_MARKER", "P4_NOM")
local name = trim(txt)
if name == "" then
    name = "NOTE"
end
reaper.DeleteExtState("RADIO_MARKER", "P4_NOM", false)

local position = reaper.GetPlayPosition()
local color = color_from_ext("P4_NOM_COLOR", "FF8C00")
